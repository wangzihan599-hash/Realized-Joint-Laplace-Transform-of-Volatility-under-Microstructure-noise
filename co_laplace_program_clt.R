...
covafunV = function(u, v, y1, y2, theta, Ti, y1preave, y2preave, omegahat12, omegahat22){
  gfun = function(u){ pmin(u, 1-u) }
  ny = length(y1)
  kn = max(3, as.integer(theta*(ny^(1/2)))) 
  ln = max(3, as.integer(theta*(ny^(1/2+1/10))))
  ...
  deltay2 = diff(y2)
  weight = gfun(u=1:(kn-1)/kn)
  #ypreave = preavefun$xpreave 
  deltan = max(Ti)/ny
  phi2 = sum(weight^2)/kn
  phi1 = 1
  D = NULL
  for (i in 1:(ny-kn-ln)){
    D = c(D, (cos(2*sqrt(2*u)*y1preave[i]/sqrt(phi2*deltan*kn))+cos(2*sqrt(2*v)*y2preave[i]/sqrt(phi2*deltan*kn))
              +cos(2*sqrt(2*u)*y1preave[i]/sqrt(phi2*deltan*kn))*cos(2*sqrt(2*v)*y2preave[i+kn]/sqrt(phi2*deltan*kn))
              -4*cos(2*sqrt(u)*y1preave[i]/sqrt(phi2*deltan*kn))*cos(2*sqrt(v)*y2preave[i+kn]/sqrt(phi2*deltan*kn))+1)
          *exp(2*u*phi1*omegahat12[i]/(phi2*theta^2))*exp(2*v*phi1*omegahat22[i]/(phi2*theta^2)))
  }
  covv = deltan/2*sum(D)
  list(covv = covv)
}
...
mainfun = function(n=23400, u=1.2, v=1.2, alpha1=0.5, alpha2=0.5,  omega1=0.001, omega2=0.0001, theta=0.06,  gam=0.3){
  ...
  y1preave = res1$y1preave  #vector
  y2preave = res1$y2preave
  omegahat12 = res1$omegahat12
  omegahat22 = res1$omegahat22
  cova = covafunV(u=u, v=v, y1=Y1, y2=Y2, theta=theta, Ti=T, y1preave=y1preave, y2preave=y2preave, omegahat12=omegahat12, omegahat22=omegahat22) 
  covariance = cova$covv  #number
  dT = diff(c(0, T))
  truepar = sum(exp(-u*sig1^2-v*sig2^2)*dT)  #number
  kn = max(3, as.integer(theta*(n^(1/2))))
  deltan =  1/n
  result = 1/(sqrt(deltan*kn))*(preaveresult-truepar)/((covariance)^(1/2))
  #standarddeviation = sqrt(covariance*kn*deltan)
  #gamma = sqrt(covariance)
  list(result = result,  truepar=truepar, preaveresult=preaveresult)
}
...
repfun = function(N, n=23400, u=1.2, v=1.2,  alpha1=0.5, alpha2=0.5, omega1=0.001, omega2=0.001, theta=0.6,  gam=0.3){
  ...
  relbias.med = median( (preaveresult-truepar)/truepar )
  se = sd(preaveresult)
  mse = mean( (preaveresult-truepar)^2 )
  
  bias = median(preaveresult-truepar)
  relbias.se = sd((preaveresult-truepar)/truepar )
  relbias.mse = mean(((preaveresult-truepar)/truepar)^2)
  
  output = rbind( cbind(bias,  se, mse, relbias.med, relbias.se, relbias.mse))
  colnames(output) = c("bias.med", "sd", "mse", "relbias.med" , "relbias.se", "relbias.mse")
  list(estclt = estclt, output=output)
}
